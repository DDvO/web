...
Dazu genügt im einfachsten Fall, die Verbindung über einen Schalter
manuell zu steuern, nämlich wenn die Batteriespannung (z.B. 24 V)
...
https://www.youtube.com/watch?v=yOcoux9IbzM[Video von Andreas Schmitz], oder
ein regelbares Netzteil, das an den Ausgang einer Powerstation gehängt wird,
siehe https://www.youtube.com/watch?v=ZXHAXrJS9CU[sein Nachfolge-Video].

Allerdings sollte in jedem Fall eine automatische Abschaltung vorgesehen werden,
wenn die Batterieladung zur Neige geht (bei LiFePO4 spätestens bei 90%
Entladung), damit die Batterie nicht durch Tiefentladung geschädigt wird.
Außerdem wäre es v.A. im Sinne der Stromkosten-Optimierung schön,
wenn die Einspeisung der gespeicherten Energie automatisch nur so stark erfolgt,
wie sie auch benötigt wird, was reglungsrechnisch nicht ganz einfach ist.
