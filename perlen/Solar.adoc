...
Genaueres kann man https://re.jrc.ec.europa.eu/pvg_tools/de/[standortspezifisch berechnen].
...
haben meist eine recht geringen Nennleistung von etwa 600-700 Wp.
Das hat neben der geringen Größe und sehr überschaubaren Kosten auch damit zu
tun, dass solche Anlagen für die private Anwendung genehmigungsfrei sind,
wenn wie weniger als 600 W in die Steckdose einspeisen.
Die nominale Leistung der verwendeten Solarmodule kann und sollte aber
durchaus größer sein (z.B. 800-1000 Wp), denn in der Praxis wird diese kaum
erreicht, vor Allem in sonnenarmen Zeiten
(und zudem hat der Solarwechselrichter auch Verluste).
Man hat dann z.B. auch im Winter entsprechend mehr Ausbeute (eigentlich sogar
überproportional mehr, weil die Schwellspannung schneller erreicht wird),
allerdings auf recht niedrigem absoluten Niveau, so dass sich deutlich mehr
Modulleistung bei Wechselrichtern, die den abgegebenen Strom auf 600 W begrenzen,
kaum lohnt.
